 ...
 * state and ends when it reachs a reward state for the current
 * task. If i is the current iteration then the current task is
 * i%taskcount.
...
double *reward = NULL;
double *q = NULL;
unsigned long int *acc_rew = NULL;
double explore;
...
int main(int argc, char **argv)
{
    ...
    acc_rew = (unsigned long int *)malloc(sizeof(unsigned long int)*task);
    ...
    fprintf(fs, "reward_sum = [\n");
    for(i = 0; i < task; ++i)
        fprintf(fs, "  %lu", acc_rew[i]);
    fprintf(fs, "\n];\n");
...
/* Returns the task with the lowest accumalted reward, ties broken by selecting
 * one of the lowest tasks randomly.
 */
unsigned int find_min_reward()
{
    unsigned long int min = acc_rew[0];
    unsigned int t = 0, c = 1;
    unsigned int *list;
    int i;
    ...
    if(c != 1){
        t = 0;
        list = (unsigned int *)malloc(sizeof(unsigned int)*c);
        for(i = 0; i < task; ++i){
            if(min == acc_rew[i])
                list[t++] = i;
        }
        t = list[(int)gsl_rng_uniform(rng)*c];
    }
    
    return t;
}

/* The Q-learning algoritim. TODO: update comment for mtqlsg
...
void mtqlrg()
{
    ...
    double temp;
    double best;
    unsigned int t = gsl_rng_uniform(rng)*task;
    
    // select starting state
    do pos = (unsigned long int)floor(gsl_rng_uniform(rng)*state);
        while(reward[pos*task + t]);
    
    while(reward[pos*task + t] == 0){
        ...
        if(gsl_rng_uniform(rng) < explore){
            ...
            for(i = 0; i < action; ++i){
                temp = q[t*action*state + pos*action + i];
                if(temp >= best){
                    best = temp;
                    azi = i;
                }
        ...
        for(i = 0; i < task; ++i){
            ...
            // also accumalate rewards.
            acc_rew[i] += reward[dest*task + i];
        }
        pos = dest;
    }
